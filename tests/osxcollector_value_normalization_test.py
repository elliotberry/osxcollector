...
def _convert_to_utc(func):
    def wrapper(dt):
        aware = datetime.fromtimestamp(time.mktime(dt.timetuple()), tz=UTC)
        return func(aware.replace(tzinfo=None))
...
class TestSecondsSince2001ToDatetime:
...
class TestSecondsSinceEpochToDatetime:
...
class TestMicrosecondsSinceEpochToDatetime:
...
class TestMicrosecondsSince1601ToDatetime:
